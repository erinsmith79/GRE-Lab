...
fortify(chrIdf)
library(ggplot2)
ggplot(data=chrIdf,mapping=aes(x=start,y=score))+geom_point()
plot_nuc_occup<-function(chrom="chrI", startpos=1, endpos=1000) {
  ...
  score<-as.numeric(positionspecif$score)
  positionspecif<-df[start>=startpos & start<=endpos,]
  fortify(positionspecif)
  ggplot(data=positionspecif,mapping=aes(x=start,y=score))+geom_point()+geom_line()
}
length(mergeG_list$chrII)
plot_nuc_occup(chrom="chrII",startpos=80,endpos=900)
df<-as.data.frame(mergeG_list$chrII)
head(df)
start<-as.numeric(df$start)
end<-as.numeric(df$end)
positionspecif<-df[start>=80 & start<=400,]
fortify(positionspecif)
score<-as.numeric(positionspecif$score)
ggplot(data=positionspecif,mapping=aes(x=start,y=score))+geom_point()+geom_line()
